...
把这个干净仓库发布到 GitHub 后，可以把它安装或克隆到你的 skills 目录。具体路径取决于你的运行环境；关键是安装后的目录里要包含 `SKILL.md`、`agents/openai.yaml` 和项目文件。
...
AI 生成的报告和分析内容仅供参考。对外使用或分享前，请人工核验来源可信度、联系方式、账号匹配关系和推断结论。
